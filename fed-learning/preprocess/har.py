...
class HARDataset(Dataset):
    ...
    def load_data(self):
        ...
        labels = (np.loadtxt(labels_file)-1).astype(int)  # Shape: (num_samples,)

        if self.transform:
            data = torch.tensor(data, dtype=torch.float32)   # Shape: (num_samples, 128, 9)
    ...
    def __getitem__(self, idx):
        ...
        sample = self.data[idx].clone().detach()
